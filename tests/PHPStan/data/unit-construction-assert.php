...
assertType("1.5&unit_float<'meter'>", unit(1.5, 'meter'));
assertType("1.5&unit_float<'meter'>", unit(1.5, NativeUnitExpressionConstants::DISTANCE));
assertType("1.5&unit_float<'meter'>", unit(1.5, YUMEMI_TEST_UNIT_PREFIX . 'ter'));
assertType("1.5&unit_float<'meter'>", unit(unit: 'meter', value: 1.5));
assertType("3&unit_int<'second'>", unit(3, 'second'));
assertType("1&unit_int<'kilogram * meter / second'>", unit(1, 'meter / second kilogram'));
assertType("4500.0&unit_float<'kilogram * meter / second ^ 2'>", unit(1500.0, 'kilogram') * unit(3.0, 'meter / second^2'));
assertType("1&unit_int<'pixel'>", unit(1, 'pixels'));
assertType("12.0&unit_float<'pixel ^ 2'>", unit(3.0, 'pixel') * unit(4.0, 'pixel'));
assertType("96.0&unit_float<'pixel / international_inch'>", unit(96.0, 'pixel / inch'));
assertType("1.0&unit_float<'english_metric_unit'>", unit(1.0, 'EMU'));
assertType("72.0&unit_float<'typographic_point'>", unit_to(96.0, 'css_pixel', 'typographic_point'));
...
function finiteUnits(string $unit): void
{
    ...
    assertType("1.0&unit_float<'international_foot'>|1.0&unit_float<'meter'>", unit(1.0, $unit));
}
...
assertType("3.2808398950131235&unit_float<'international_foot / meter'>", unit_factor('meter', 'foot'));
assertType("3.2808398950131235&unit_float<'international_foot'>", unit(1, 'meter') * unit_factor('meter', 'foot'));
assertType("3.2808398950131235&unit_float<'international_foot'>", unit_factor('meter', 'foot') * unit(1, 'meter'));
assertType("0.3048&unit_float<'meter'>", unit(1.0, 'foot') * unit_factor('foot', 'meter'));
assertType("3.6&unit_float<'1000 * meter / hour'>", unit(1, 'meter / second') * unit_factor(
    'meter / second',
    'kilometer / hour',
));
assertType("1.0&unit_float<'1'>", unit_factor('meter', 'meter'));
assertType("1.0&unit_float<'meter'>", unit(1, 'meter') * unit_factor('meter', 'meter'));
assertType("100.0&unit_float<'1/100'>", unit_factor('meter', 'centimeter'));
assertType("100.0&unit_float<'1/100 * meter'>", unit(1, 'meter') * unit_factor('meter', 'centimeter'));

/** @param 'meter'|'foot' $from */
function finiteFactorSource(string $from): void
{
    assertType("0.3048&unit_float<'meter / international_foot'>|1.0&unit_float<'1'>", unit_factor($from, 'meter'));
}

/** @param 'meter'|'foot' $to */
function finiteFactorTarget(string $to): void
{
    assertType("1.0&unit_float<'1'>|3.2808398950131235&unit_float<'international_foot / meter'>", unit_factor('meter', $to));
}

/** @param 'foot'|'international_foot' $from */
function equivalentFactorSource(string $from): void
{
    assertType("0.3048&unit_float<'meter / international_foot'>", unit_factor($from, 'meter'));
}

/** @param '100 * centimeter'|'meter' $from */
function definitionallyEquivalentFactorSource(string $from): void
{
    assertType("1.0&unit_float<'1'>", unit_factor($from, 'meter'));
}
...
assertType("0.9144000000000001&unit_float<'meter'>", unit_to(3.0, 'foot', 'meter'));
assertType("3.2808398950131235&unit_float<'international_foot'>", unit_to(1.0, 'meter', 'foot'));
assertType("2.54&unit_float<'1/100 * meter'>", unit_to(1.0, 'inch', 'centimeter'));
assertType("0.45359237&unit_float<'kilogram'>", unit_to(1.0, 'pound', 'kilogram'));
assertType("3600.0&unit_float<'second'>", unit_to(1.0, 'hour', 'second'));
assertType("26.8224&unit_float<'meter / second'>", unit_to(60.0, 'mile / hour', 'meter / second'));
assertType("3.6&unit_float<'1000 * meter / hour'>", unit_to(1.0, 'meter / second', 'kilometer / hour'));
assertType("0.001&unit_float<'meter ^ 3'>", unit_to(1.0, 'liter', 'meter^3'));
assertType("1000.0&unit_float<'gram'>", unit_to(1.0, 'kilogram', 'gram'));
assertType("1000.0&unit_float<'meter'>", unit_to(1.0, 'kilometer', 'meter'));

// affine sources can convert into a multiplicative brand; affine targets remain plain float
assertType("273.15&unit_float<'kelvin'>", unit_to(0, 'celsius', 'kelvin'));
assertType('float', unit_to(273.15, 'kelvin', 'celsius'));
assertType('212.0', unit_to(100, 'celsius', 'fahrenheit'));

// factor-1 derived SI still rebrands to the *to* spelling
assertType("1.0&unit_float<'kilogram * meter / second ^ 2'>", unit_to(1.0, 'newton', 'kilogram * meter / second^2'));
assertType("1.0&unit_float<'newton'>", unit_to(1.0, 'kilogram * meter / second^2', 'newton'));
assertType("1.0&unit_float<'joule'>", unit_to(1.0, 'newton * meter', 'joule'));
assertType("1.0&unit_float<'pascal'>", unit_to(1.0, 'newton / meter^2', 'pascal'));

// identity / alias
assertType("5.0&unit_float<'meter'>", unit_to(5.0, 'meter', 'meter'));
assertType("2.0&unit_float<'1000 * meter'>", unit_to(2.0, 'kilometer', '1000 * meter'));

// branded value + matching from
assertType("0.9144000000000001&unit_float<'meter'>", unit_to(unit(3.0, 'foot'), 'foot', 'meter'));
assertType("26.8224&unit_float<'meter / second'>", unit_to(unit(60.0, 'mile / hour'), 'mile / hour', 'meter / second'));

// int magnitude still yields unit_float after conversion
assertType("0.3048&unit_float<'meter'>", unit_to(12, 'inch', 'meter'));

/** @param 'foot'|'meter' $from */
function finiteUnitToSource(string $from): void
{
    $converted = unit_to(1.0, $from, 'inch');

    assertType("12.0&unit_float<'international_inch'>|39.37007874015748&unit_float<'international_inch'>", $converted);
    acceptFiniteConvertedLength($converted);
}

/** @param unit_float<'international_inch'> $length */
function acceptFiniteConvertedLength(float $length): void {}

/** @param 'foot'|'meter' $to */
function finiteUnitToTarget(string $to): void
{
    assertType("0.0254&unit_float<'meter'>|0.08333333333333333&unit_float<'international_foot'>", unit_to(1.0, 'inch', $to));
}

/** @param 'foot'|'international_foot' $to */
function equivalentUnitToTarget(string $to): void
{
    assertType("0.08333333333333333&unit_float<'international_foot'>", unit_to(1.0, 'inch', $to));
}

/** @param '100 * centimeter'|'meter' $to */
function definitionallyEquivalentUnitToTarget(string $to): void
{
    assertType("0.3048&unit_float<'meter'>", unit_to(1.0, 'foot', $to));
}
...
{
    assertType("255.92777777777778&unit_float<'kelvin'>|274.15&unit_float<'kelvin'>", unit_to(1.0, $from, 'kelvin'));
}
